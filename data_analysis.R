...
afb$weather_extreme <- ifelse(afb$spei_24_d <= -0.5|afb$spei_24_d >= 0.5, 1, 0)
afb$dry <- ifelse(afb$spei_24_d <= -0.5, 1, 0)
afb$wet <- ifelse(afb$spei_24_d >= 0.5, 1, 0)

afb$weather_extreme6 <- ifelse(afb$spei_6_d <= -0.5|afb$spei_6_d >= 0.5, 1, 0)
afb$weather_extreme12 <- ifelse(afb$spei_12_d <= -0.5|afb$spei_12_d >= 0.5, 1, 0)
afb$weather_extremei <- ifelse(afb$spei_12_i <= -0.5|afb$spei_12_i >= 0.5, 1, 0)
...
base4 <- felm(refuse_to_pay ~ weather_extreme + age + I(age^2) |
...
base5 <- felm(refuse_to_pay ~ weather_extreme + age + I(age^2) |
                region + year_month + country_year + gender + edu + rural_sample + race + employ + occupation |
                0 | cluster, data = afb)

final <- felm(refuse_to_pay ~ weather_extreme +
...
stargazer(models, type = "text",
          ...
          covariate.labels = c("Extreme"),
          omit.stat = c("rsq", "adj.rsq", "ser"))
stargazer(models, type = "html",
          out = "Output/Tables/average_effect.doc",
          omit = c("age", "n_", "6", "12", "i", "sd"),
          digits = 3,
          dep.var.caption = "Dependent variable: Refuse to Pay",
          dep.var.labels.include = FALSE,
...
# Model 1: Base model without region and month fixed effects
base1 <- felm(refuse_to_pay ~ weather_extreme +
                edry6 + ewet6 + ldry6 + lwet6 +
                edry12 + ewet12 + ldry12 + lwet12 +
                edryi + eweti + ldryi + lweti +
                n_drought_2ys + n_flood_2ys + n_earthquake_2ys + 
...
# Model 2: Base model with region fixed effects
base2 <- felm(refuse_to_pay ~ dry + wet + 
                edry6 + ewet6 + ldry6 + lwet6 +
                edry12 + ewet12 + ldry12 + lwet12 +
                edryi + eweti + ldryi + lweti +
                n_drought_2ys + n_flood_2ys + n_earthquake_2ys + 
...
# Model 3: Base model with region and month fixed effects
base3 <- felm(refuse_to_pay ~ edry + ewet + ldry + lwet +
                edry6 + ewet6 + ldry6 + lwet6 +
                edry12 + ewet12 + ldry12 + lwet12 +
                edryi + eweti + ldryi + lweti +
                n_drought_2ys + n_flood_2ys + n_earthquake_2ys + 
...
# Model 4: Base model with region, month, and country-year fixed effects
final_probit <- feglm(
  refuse_to_pay ~ edry + ewet + ldry + lwet + edry6 + ewet6 + ldry6 + 
    lwet6 + edry12 + ewet12 + ldry12 + lwet12 + edryi + eweti + 
    ldryi + lweti + n_drought_2ys + n_flood_2ys + n_earthquake_2ys + 
    n_extreme_temp_2ys + n_storm_2ys + sd_24m + age + I(age^2) | 
    region + year_month + country_year + gender + edu + rural_sample + 
    race + employ + occupation,
  data = afb,
...
etable(final_probit)
models <- list(base1, base2, base3,final_probit)
...
for (dep_var in dependent_vars) {
  ...
  # Model 1: weather_extreme
  formula_extreme <- as.formula(paste0(
    dep_var, " ~ weather_extreme +
    age + I(age^2) + sd_24m +
    n_drought_2ys + n_flood_2ys + n_earthquake_2ys + 
    n_extreme_temp_2ys + n_storm_2ys |
    edry6 + ewet6 + ldry6 + lwet6 +
  ...
  # Model 2: edry, ewet, ldry, lwet
  formula_drywet <- as.formula(paste0(
  ...
  models[[paste0(dep_var, "_extreme")]] <- felm(formula_extreme, data = afb)
  models[[paste0(dep_var, "_drywet")]] <- felm(formula_drywet, data = afb)
}

# Order models for table
models_out <- models[c(
  "would_do_extreme",
  "would_do_drywet",
  "behavior_extreme",
...
stargazer(models_out, type = "text",
          keep = c("dry", "wet","weather_extreme"),
          digits = 3,
          dep.var.caption = "Dependent variable: Refuse to Pay",
          dep.var.labels.include = FALSE,
          add.lines = list(
          ...
          covariate.labels = c("Extreme","Exceptional Drought", "Exceptional Wet", "Limited Drought", "Limited Wet"),
...
p_heterogeneity_occupation <- ggplot(results_df, aes(x = estimate, y = occupation, color = term)) +
  ...
  geom_errorbar(aes(xmin = conf.low, xmax = conf.high), 
                ...
                linewidth = 2, show.legend = FALSE) +  # Larger error bars, no legend for error bars
...
p_heterogeneity_occupation=p_heterogeneity_occupation+
  ggtitle("A. Occupational Heterogeneity")
...
afb$lower_edu <- ifelse(
  is.na(afb$edu),
  NA,
  ifelse(afb$edu %in% c("No Formal Education", "Primary"), 1, 0)
)
lower_edu <- felm(
...
p_vul_hardship=p_vul_hardship+
  ggtitle("B. Weather Extremes and Life Hardship")
...
p_econ=p_econ+
  ggtitle("C. Weather Extremes and Perceptions of Economic Trends")
...
ggsave("Output/Figures/p_econ_channel.jpg",p_econ_channel, width = 15.2, height = 20)
...
afb$occupation_ref <- factor(afb$occupation)
afb$occupation_ref <- relevel(afb$occupation_ref, ref = "Formal Employment & Professionals")

afb$race_ref <- factor(afb$race)
afb$race_ref <- relevel(afb$race_ref, ref = "Black")

selection_model <- feols(
  weather_extreme ~
    gender + age + edu + rural_sample + race_ref + employ + occupation_ref |
    region + year_month + country_year,
  cluster = ~cluster,
  data = afb
)

etable(selection_model)

final <- felm(weather_extreme ~ gender + age + edu + rural_sample + 
                race_ref + employ + occupation_ref  |
                region + year_month + country_year |
                0 | cluster, data = afb)
...
m1 <- feglm(refuse_to_pay ~ weather_extreme +
              n_drought_2ys + n_flood_2ys + n_earthquake_2ys + 
              n_extreme_temp_2ys + n_storm_2ys + sd_24m+
              age + I(age^2) |
              edry6 + ewet6 + ldry6 + lwet6 +
...
m2 <- feglm(refuse_to_pay ~ edry + ewet + ldry + lwet +
              n_drought_2ys + n_flood_2ys + n_earthquake_2ys + 
              n_extreme_temp_2ys + n_storm_2ys + sd_24m+
              age + I(age^2) |
              edry6 + ewet6 + ldry6 + lwet6 +
...
etable(m1,m2,
       keep = c("weather_extreme", "edry","ewet","ldry","lwet"),
       digits = 3)
etable(
  m1,
  m2,
  keep = "weather_extreme|edry|ewet|ldry|lwet",
  type = "html",
  file = "Output/Tables/robustness_probit.doc"
)
library(marginaleffects)

ame1 <- avg_slopes(
  m1,
  variables = "weather_extreme",
  vcov=FALSE
)

ame2 <- avg_slopes(
  m2,
  variables = c("edry", "ewet", "ldry", "lwet"),
  vcov=FALSE
)
...
ame_table
ame_table <- ame_table %>% dplyr::select(-contrast)%>% 
  dplyr::mutate(across(where(is.numeric), ~ round(.x, 3)))
write.csv(
  ame_table,
  "Output/Tables/probit_marginal.csv",
  row.names = FALSE
)
...
m_6 <- felm(
  refuse_to_pay ~ edry6 + ewet6 + ldry6 + lwet6 +
    n_drought_2ys + n_flood_2ys + n_earthquake_2ys + 
    n_extreme_temp_2ys + n_storm_2ys + sd_24m +
    age + I(age^2) |
    region + year_month + country_year + 
...
m_12 <- felm(
  refuse_to_pay ~ edry12 + ewet12 + ldry12 + lwet12 +
    n_drought_2ys + n_flood_2ys + n_earthquake_2ys + 
    n_extreme_temp_2ys + n_storm_2ys + sd_24m +
    age + I(age^2) |
    region + year_month + country_year + 
...
m_24 <- felm(
  refuse_to_pay ~ edry + ewet + ldry + lwet +
    n_drought_2ys + n_flood_2ys + n_earthquake_2ys + 
    n_extreme_temp_2ys + n_storm_2ys + sd_24m +
    age + I(age^2) |
    region + year_month + country_year + 
...
m_interview <- felm(
  refuse_to_pay ~ edryi + eweti + ldryi + lweti +
    n_drought_2ys + n_flood_2ys + n_earthquake_2ys + 
    n_extreme_temp_2ys + n_storm_2ys + sd_24m +
    age + I(age^2) |
    region + year_month + country_year + 
...
models <- list(m_6, m_12, m_24)

stargazer(models, type = "text",
          keep=c("weather_extreme"),
          digits = 3,
          dep.var.caption = "Dependent variable: Refuse to Pay",
          dep.var.labels.include = FALSE,
          add.lines = list(c('Region FE', 'Y', 'Y', 'Y', 'Y', 'Y'),
                           ...
                           c('Country-Year FE', 'Y', 'Y', 'Y', 'Y', 'Y'),
                           c('Control', 'Y', 'Y', 'Y', 'Y', 'Y')),
          covariate.labels = c("Extreme (6 months)","Extreme (12 months)","Extreme (24 months)"),
          omit.stat = c("rsq", "adj.rsq", "ser"))
stargazer(models, type = "html",
          out = "Output/Tables/time_lags_6_12_24.doc",
          keep=c("weather_extreme"),
          digits = 3,
          dep.var.caption = "Dependent variable: Refuse to Pay",
          dep.var.labels.include = FALSE,
...
# 1. Define region identifier
# Use country + region if region names/codes may repeat across countries.
afb_var <- afb %>%
  mutate(
    weather_extreme = as.numeric(weather_extreme)
  ) %>%
  filter(!is.na(weather_extreme), !is.na(region))

# 2. Residualize the pooled hydroclimatic anomaly indicator by region fixed effects
res_model <- feols(
  weather_extreme ~ 1 | region,
  data = afb_var
)

# 3. Extract residuals
afb_var$resid_weather_extreme <- resid(res_model)

# 4. Calculate share of total variance retained after removing region fixed effects
variance_retained <- var(afb_var$resid_weather_extreme, na.rm = TRUE) /
  var(afb_var$weather_extreme, na.rm = TRUE)
